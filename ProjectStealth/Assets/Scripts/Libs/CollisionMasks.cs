...
public static class CollisionMasks
{
    ...
    public static int all_collision_mask = geo_mask | object_mask | character_mask | jump_through_mask | enemy_mask;
    public static int upwards_collision_mask = geo_mask | object_mask | character_mask;
    public static int wall_grab_mask = geo_mask; // TODO: may want a new layer for this one.

    // light
    // ASSUMES: all light-blocking geometry is on these layers. Should log an error if you try another layer.
    public static int light_occlusion_mask = geo_mask | object_mask | default_mask;
